...
class DatabaseHelperd {
  ...
  DatabaseHelperd._privateConstructor();

  static final DatabaseHelperd instance = DatabaseHelperd._privateConstructor();
  ...
  // inserted row.
  Future<int> insert(Caru car) async {
  ...
  Future<int> update(Caru car) async {
    Database db = await instance.database;
    int id = car.toMap()['id'];
  ...
  Future<List<Map<String, dynamic>>> queryRow(name) async {
    Database db = await instance.database;
    return await db.query(table, where: "$columnNames LIKE '%$name%'");
  }
}
